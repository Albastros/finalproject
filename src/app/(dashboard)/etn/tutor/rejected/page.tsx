...
export default function RejectedTutorsPage() {
  ...
  const fetchRejectedTutors = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/api/etn/tutors?rejected=true");
      const tutorsData = response.data.tutors || [];
      setTutors(tutorsData);
    } catch (error) {
      console.error("Failed to fetch rejected tutors:", error);
      toast.error("Failed to load rejected tutors");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-lg">Applications that were declined</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-border rounded-xl bg-background focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  />
                </div>
              </div>
            </CardContent>
        ...
        {filteredTutors.length === 0 ? (
          <Card className="border-0 shadow-lg">
            <CardContent className="p-16 text-center">
              ...
              <h3 className="text-xl font-semibold mb-2">No Rejected Tutors Found</h3>
              <p className="text-muted-foreground">
                {searchTerm ? "Try adjusting your search criteria" : "No tutors have been rejected yet"}
              </p>
            </CardContent>
          </Card>
        ) : (
              ...
              {paginatedTutors.map((tutor) => (
                <Card key={tutor._id} className="group hover:shadow-2xl transition-all duration-500 border-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm overflow-hidden">
                  <CardContent className="p-6 space-y-4">
                    ...
                    {/* Profile */}
                    <div className="flex items-center gap-3">
                      <Avatar className="w-12 h-12 ring-2 ring-red-200 dark:ring-red-800">
                    ...
                    <div className="space-y-2 text-sm">
                      {tutor.subjects && tutor.subjects.length > 0 && (
                        <div className="flex items-center gap-2">
                          ...
                          {tutor.subjects.length > 2 && <span className="text-muted-foreground">+{tutor.subjects.length - 2}</span>}
                        </div>
                      )}
                      {tutor.experience && (
                        <div className="flex items-center gap-2">
                      ...
                      {tutor.price && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>{tutor.price} birr/hour</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1 text-xs text-muted-foreground pt-2 border-t">
                      ...
                      Applied {tutor.createdAt ? new Date(tutor.createdAt).toLocaleDateString() : 'N/A'}
                    </div>

                    {/* View Details Button */}
                    <Button 
                      onClick={() => setSelectedTutor(tutor)}
                      className="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white shadow-lg hover:shadow-xl transition-all duration-300"
                    >
        ...
        <Dialog open={!!selectedTutor} onOpenChange={() => setSelectedTutor(null)}>
          <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] overflow-hidden bg-white dark:bg-slate-900 border-0 shadow-2xl">
            <DialogHeader className="border-b pb-4 bg-gradient-to-r from-red-50 to-rose-50 dark:from-slate-800 dark:to-slate-700 -m-6 mb-0 p-6">
              <DialogTitle className="text-xl font-bold bg-gradient-to-r from-red-600 to-rose-600 bg-clip-text text-transparent">
                Rejected Tutor Details
              </DialogTitle>
            </DialogHeader>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                          <div className="text-xl font-bold text-red-600">{selectedTutor.experience || 0}</div>
                          <div className="text-xs text-muted-foreground">Years Experience</div>
                        </div>
                        <div className="text-center p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                          <div className="text-xl font-bold text-red-600">{selectedTutor.price || 100}</div>
